# Module: Frontend CloudFront Distribution

Creates a CloudFront distribution backed by an S3 static website origin. Looks up an existing ACM certificate by domain name (must be in `us-east-1`). Supports managed cache/request policies, geo-restrictions, and TLS configuration.
