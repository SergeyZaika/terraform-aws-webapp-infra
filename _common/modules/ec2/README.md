...
Provisions an EC2 instance with optional Elastic IP, secondary data volume, and SSH key pair. Generates a new RSA key pair via `tls_private_key` and saves it locally, or accepts an existing key pair name. IMDSv2 is enforced by default.
